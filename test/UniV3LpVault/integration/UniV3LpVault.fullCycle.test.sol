...
contract UniV3LpVaultFullCycleTest is Test {
    ...
    function test_fullCycle_DepositMintCollectBurnWithdraw() public {
        ...
        assertTrue(setup.token0.balanceOf(recipient) > recipientBalanceBefore0);
        assertTrue(setup.token1.balanceOf(recipient) > recipientBalanceBefore1);
        assertTrue(withdrawn0 > 0);
        assertTrue(withdrawn1 > 0);

        // Verify fee collector received TVL fees
        assertTrue(setup.token0.balanceOf(setup.feeCollector) > feeCollectorBalanceBefore0);
        assertTrue(setup.token1.balanceOf(setup.feeCollector) > feeCollectorBalanceBefore1);
        ...
        helper.assertApproxEqual(finalNet0, 0, TestConstants.TOLERANCE_HIGH, "Final net assets0 should be ~0");
        helper.assertApproxEqual(finalNet1, 0, TestConstants.TOLERANCE_HIGH, "Final net assets1 should be ~0");
        helper.assertApproxEqual(finalLp0, 0, TestConstants.TOLERANCE_HIGH, "Final LP value0 should be ~0");
        helper.assertApproxEqual(finalLp1, 0, TestConstants.TOLERANCE_HIGH, "Final LP value1 should be ~0");
