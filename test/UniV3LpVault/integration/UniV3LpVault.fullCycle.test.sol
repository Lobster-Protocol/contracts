...
contract UniV3LpVaultFullCycleTest is Test {
    ...
    function test_fullCycle_DepositMintCollectBurnWithdraw() public {
        ...
        (uint256 netAfterDeposit0, uint256 netAfterDeposit1) = setup.vault.netAssetsValue();
        ...
        (uint256 netWithPendingFees0, uint256 netWithPendingFees1) = setup.vault.netAssetsValue();
        ...
        assertTrue(setup.token0.balanceOf(address(setup.vault)) >= vaultBalanceBeforeCollect0);
        assertTrue(setup.token1.balanceOf(address(setup.vault)) >= vaultBalanceBeforeCollect1);
        ...
        helper.assertApproxEqual(
            uint256(pos1_after_burn.liquidity), uint256(pos1.liquidity - burnAmount), 1, "Position liquidity after burn"
        );
        ...
        uint256 feeCollectorBalanceBefore1 = setup.token1.balanceOf(setup.feeCollector);
        ...
        (uint256 withdrawn0, uint256 withdrawn1) = helper.withdrawFromVault(setup, withdrawPercentage, recipient);

        uint256 tvlFeePercent = setup.vault.tvlFeeScaled().mulDiv(delay, 365 days);
        ...
        assertEq(setup.token0.balanceOf(recipient), recipientBalanceBefore0 + withdrawn0);

        assertEq(setup.token1.balanceOf(recipient), recipientBalanceBefore1 + withdrawn1);

        assertTrue(withdrawn0 > 0);
        assertTrue(withdrawn1 > 0);

        // Verify fee collector received TVL fees
        assertApproxEqAbs(setup.token0.balanceOf(setup.feeCollector), feeCollectorBalanceBefore0 + expectedFee0, 1);

        assertApproxEqAbs(setup.token1.balanceOf(setup.feeCollector), feeCollectorBalanceBefore1 + expectedFee1, 1);

        // === PHASE 8: More Time Passes ===
        helper.simulateTimePass(TestConstants.ONE_WEEK);

        // === PHASE 9: Final Full Withdrawal ===
        uint256 recipientBalanceBeforeFinal0 = setup.token0.balanceOf(recipient);
        uint256 recipientBalanceBeforeFinal1 = setup.token1.balanceOf(recipient);
        ...
        assertTrue(setup.token0.balanceOf(recipient) > recipientBalanceBeforeFinal0);
        assertTrue(setup.token1.balanceOf(recipient) > recipientBalanceBeforeFinal1);

        // Verify additional TVL fees collected
        assertTrue(setup.token0.balanceOf(setup.feeCollector) >= feeCollectorBalanceBeforeFinal0);
        assertTrue(setup.token1.balanceOf(setup.feeCollector) >= feeCollectorBalanceBeforeFinal1);
        ...
        uint256 totalFeesCollected0 = setup.token0.balanceOf(setup.feeCollector);
        uint256 totalFeesCollected1 = setup.token1.balanceOf(setup.feeCollector);
        ...
        uint256 totalAccountedFor0 = userFinalBalance0 + totalRecipientReceived0 + totalFeesCollected0 + vaultRemainder0;
        uint256 totalAccountedFor1 = userFinalBalance1 + totalRecipientReceived1 + totalFeesCollected1 + vaultRemainder1;

        helper.assertApproxEqual(
            totalAccountedFor0, userInitialBalance0, TestConstants.TOLERANCE_MEDIUM, "Token0 conservation check failed"
        );
        helper.assertApproxEqual(
            totalAccountedFor1, userInitialBalance1, TestConstants.TOLERANCE_MEDIUM, "Token1 conservation check failed"
        );

        // === PHASE 12: Verify Reasonable Fee Collection ===
        // TVL fees should be reasonable (less than 10% of initial deposit for ~5 weeks)
        assertTrue(totalFeesCollected0 < initialDeposit0 / 10, "TVL fees seem too high for token0");
        assertTrue(totalFeesCollected1 < initialDeposit1 / 10, "TVL fees seem too high for token1");
        assertTrue(totalFeesCollected0 > 0, "Should have collected some TVL fees for token0");
        assertTrue(totalFeesCollected1 > 0, "Should have collected some TVL fees for token1");
    }
    ...
    function test_fullCycle_NoPositionsOnlyCash() public {
        ...
        uint256 feeCollectorBefore1 = setup.token1.balanceOf(setup.feeCollector);

        // Withdraw 25%
        helper.withdrawFromVault(setup, TestConstants.QUARTER_SCALED_PERCENTAGE, recipient);

        // TVL fees should be collected
        assertTrue(setup.token0.balanceOf(setup.feeCollector) > feeCollectorBefore0);
        assertTrue(setup.token1.balanceOf(setup.feeCollector) > feeCollectorBefore1);
        ...
        (uint256 finalNet0, uint256 finalNet1) = setup.vault.netAssetsValue();
        assertApproxEqAbs(finalNet0, 0, 1);
