...
contract UniswapV3VaultFlow is IVaultFlowModule, INav {
    ...
    function _withdraw(
        ...
        if (
            ...
            uint256 slippageDiffBasisPoint = twPrice.mulDiv(
                BASIS_POINT_SCALE,
                spotPrice
            ); // < 10_000 since spotPrice > twPrice
        ...
        console.log(
            "vaultAsset.balanceOf(address(vault)) < assets",
            vaultAsset.balanceOf(address(vault)) < assets
        );
        ...
        if (tokensCount > 0 && vaultAsset.balanceOf(address(vault)) < assets) {
            ...
            uint256 sharesRatio = shares.mulDiv(
                BASIS_POINT_SCALE,
                vault.totalSupply()
            );
            ...
            uint256 allPosition0 = 0;
            uint256 allPosition1 = 0;
            uint256 allFee0 = 0;
            uint256 allFee1 = 0;
            ...
            for (uint256 i = 0; i < tokensCount; i++) {
                ...
                (
                    ...
                    uint256 tokensOwed0,
                    uint256 tokensOwed1
                ) = positionManager.positions(tokenId); // todo: merge with PositionValue.total to also get the token0 & token1 amounts and save gas
                ...
                console.log("position0", position0, "fee0", fee0);
                console.log("position1", position1, "fee1", fee1);

                // save the withdrawn values
                allPosition0 += position0;
                allPosition1 += position1;
                allFee0 += fee0;
                allFee1 += fee1;
                ...
                uint256 toWithdrawFee0 = fee0
                    .mulDiv(sharesRatio, BASIS_POINT_SCALE)
                    .mulDiv(
                        BASIS_POINT_SCALE - feeCutBasisPoint,
                        BASIS_POINT_SCALE
                    );
                uint256 toWithdrawFee1 = fee1
                    .mulDiv(sharesRatio, BASIS_POINT_SCALE)
                    .mulDiv(
                        BASIS_POINT_SCALE - feeCutBasisPoint,
                        BASIS_POINT_SCALE
            ...
            uint256 amountIn = assetIsToken0
                ? token1Before + allFee1 + allPosition1
                : token0Before + allFee0 + allPosition0;
                console.log("amountIn last swap: ", amountIn);
            uint256 quote = UniswapUtils.getQuoteFromSqrtRatioX96(
                sqrtPriceX96,
                uint128(10 ** (assetIsToken0 ? decimals1 : decimals0)),
                assetIsToken0
            );
            uint256 amountOutWithSlippage = amountIn.mulDiv(
                quote * (BASIS_POINT_SCALE - MAX_SLIPPAGE),
                BASIS_POINT_SCALE
            );
            console.log(
                ...
                data: abi.encodeCall(
                    ...
                    (
                        IUniswapV3RouterMinimal.ExactInputSingleParams({
                            ...
                            deadline: block.timestamp + 1 hours,
                            amountIn: amountIn,
                            amountOutMinimum: amountOutWithSlippage - 1e10,
                            sqrtPriceLimitX96: sqrtPriceX96 - 100 // todo: preview sqrt price
                        })
                    )
                )
            });
        ...
        returns (uint256 twPrice, uint256 spotPrice)
    {
        ...
        spotPrice = UniswapUtils.getQuoteFromSqrtRatioX96(
            sqrtPriceX96,
            uint128(10 ** decimals),
            assetIsToken0
        );
